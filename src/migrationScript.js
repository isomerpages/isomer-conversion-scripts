...
async function createMigrationBranch(
  repoName = repoToMigrate,
  branchName = "v2Migration"
) {
  let res;
  try {
    // create branch
...
*/
  const config = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_config.yml"
  );
  const homepage = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_data/homepage.yml"
  );
  const navigation = await utils.getFileFromGithub(
    header,
    repoToMigrate,
  ...
  let contactUs = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_data/contact-us.yml"
  );
  if (!contactUs.content)
    contactUs = await utils.getFileFromGithub(
      header,
      repoToMigrate,
      "_data/contact-us.yaml"
    );

  const careersStories = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_data/careers-stories.yml"
  );
  const products = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_data/products.yml"
  );
  const programmes = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_data/programmes.yml"
  );
  const resources = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_data/resources.yml"
  );
  const socialMedia = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "_data/social-media.yml"
  );
  const privacyMd = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "pages/privacy.md"
  );
  const termsMd = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "pages/terms-of-use.md"
  );
  const contactUsMd = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "pages/contact-us.md"
  );
  const indexMd = await utils.getFileFromGithub(
    header,
    repoToMigrate,
    "index.md"
  );
...
*/
  ...
  const uploadFooter = await utils.updateFileOnGithub(
    header,
    repoToMigrate,
    "_data/footer.yml",
    footer
  ...
  */
  ...
  const uploadConfig = await utils.updateFileOnGithub(
    header,
    repoToMigrate,
    "_config.yml",
    newConfigFile,
  ...
  const uploadNav = await utils.updateFileOnGithub(
    header,
    repoToMigrate,
    "_data/navigation.yml",
    newNavFile,
  ...
  const uploadIndex = await utils.updateFileOnGithub(
    header,
    repoToMigrate,
    "index.md",
    newIndexFile,
  ...
  if (contactUs.content) {
    ...
    if (uploadContactUs.status === 200)
      console.log("contact-us.md was updated");
  }
  /*
...
*/
  ...
  await Bluebird.each(pagesToDel, (page) => {
    if (page.path) {
      return utils.deleteFileOnGithub(
        header,
        repoToMigrate,
        page.path,
        page.sha
  ...
  */
  try {
    ...
    await Bluebird.each(data, async (file) => {
      ...
      const isMd =
        file.name.split(".")[file.name.split(".").length - 1] === "md";
      ...
      if (file.type === "file" && isMd) {
        ...
        return utils
          .getFileFromGithub(header, repoToMigrate, file.path)
          .then(({ content, sha }) => {
            if (utils.checkFrontMatter(content)) {
              ...
              // update the file
              return utils.updateFileOnGithub(
                header,
                repoToMigrate,
                file.path,
                res,
